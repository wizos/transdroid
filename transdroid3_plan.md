...
- **Two-pane tablet layout & torrent-details screen**: `transdroid-m3-tablet.html` (light) and `transdroid-m3-tablet-dark.html` (dark) are a single wide-viewport screen combining both — a top app bar spanning both panes (title/server chip, a speed-limit toggle alongside search/refresh/sort/more, and aggregate DL/UL speed with active/sharing counts matching the phone toolbar's two-line stat format), a fixed-width list pane (filter chips + the same row anatomy as the phone list) on the left, and a torrent-details pane on the right that updates as the selected row changes. Details pane anatomy: header (state badge, name, status + label pills), a primary action row (context-sensitive play/pause/resume/"Start now", Remove, force-recheck, set-label, more), a large progress indicator with a summary line, then three tabs — Overview (a stat-tile grid: downloaded/uploaded/ratio/speeds/seeders/peers/ETA/size/added), Files (per-file rows with priority chip — High/Normal/Skip — and a mini progress bar), and Trackers (per-tracker rows with seed/peer counts and a working/idle status chip). This is the reference both for the phone-only torrent-details screen (Phase 2) and for how the list pane and details pane recombine into a `WindowSizeClass` two-pane layout on wider screens (Phase 3) — on narrower widths, the same details-pane content becomes its own full-screen destination. The dark variant follows the same dark tonal-role remapping as `transdroid-m3-expressive-dark.html` (dark surfaces, lightened primary, brightened status colors).
